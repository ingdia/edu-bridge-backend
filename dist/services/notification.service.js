...
class NotificationService {
    ...
    async createBulkNotifications(recipientIds, data) {
        const notifications = await Promise.all(recipientIds.map((recipientId) => this.createNotification({
            ...data,
            recipientId,
        })));
    ...
    async updateNotificationStatus(notificationId, studentId, status) {
        const notification = await prisma.notification.findFirst({
            where: {
                id: notificationId,
                recipientId: studentId,
    ...
    async createSessionReminder(studentId, sessionDetails, mentorId) {
    ...
    async createDeadlineAlert(studentId, deadlineDetails, adminId) {
        return await this.createNotification({
            recipientId: studentId,
            type: 'DEADLINE_ALERT',
    ...
    async createFeedbackNotification(studentId, feedbackDetails, mentorId) {
        return await this.createNotification({
            recipientId: studentId,
