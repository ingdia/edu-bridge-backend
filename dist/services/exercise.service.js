...
class ExerciseService {
    ...
    static generateFilename(studentId, moduleId, ext) {
    ...
    static async submitTextExercise(studentId, data) {
        ...
        switch (data.exerciseType) {
            case client_1.ExerciseType.LISTENING:
            case client_1.ExerciseType.READING:
                submissionContent = { responses: data.responses };
                break;
            case client_1.ExerciseType.WRITING:
                submissionContent = { content: data.content, wordCount: data.wordCount };
                break;
            case client_1.ExerciseType.DIGITAL_LITERACY:
                submissionContent = {
        ...
        const result = await database_1.default.$transaction(async (tx) => {
            ...
            const progressUpdate = await tx.progress.upsert({
                ...
                create: {
                    studentId,
                    moduleId,
                    completedAt: exerciseType === client_1.ExerciseType.DIGITAL_LITERACY && data.completed
                        ? new Date() : null,
    ...
    static async submitSpeakingExercise(studentId, moduleId, file, metadata) {
        ...
        if (!module.isActive)
            throw new Error('Cannot submit to inactive module');
        if (module.type !== client_1.ExerciseType.SPEAKING) {
        ...
        const submission = await database_1.default.exerciseSubmission.create({
            data: {
                studentId,
                moduleId,
                exerciseType: client_1.ExerciseType.SPEAKING,
                submissionContent: {
        ...
        await (0, logger_1.logAudit)(studentId, 'SPEAKING_EXERCISE_SUBMITTED', { moduleId, filename, fileSize: file.size });
    ...
    static async getMentorSubmissions(mentorId, filters) {
        ...
        const sessions = await database_1.default.mentorshipSession.findMany({
            where: { mentorId, status: { in: [client_1.SessionStatus.SCHEDULED, client_1.SessionStatus.COMPLETED] } },
        ...
        const summary = {
            ...
            pending: submissions.filter((s) => s.status === 'pending').length,
            evaluated: submissions.filter((s) => s.status === 'evaluated').length,
            byType: Object.values(client_1.ExerciseType).reduce((acc, type) => {
