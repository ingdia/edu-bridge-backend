...
exports.getApplicationStatistics = exports.getProgressOverTime = exports.getMentorEffectivenessAnalytics = exports.getModuleEngagementAnalytics = exports.getStudentPerformanceAnalytics = exports.getSystemOverview = void 0;
...
const getSystemOverview = async () => {
    const [totalStudents, totalMentors, totalAdmins, activeStudents, totalModules, activeModules, totalSubmissions, totalSessions, completedSessions,] = await Promise.all([
        database_1.default.user.count({ where: { role: 'STUDENT', isActive: true } }),
        database_1.default.user.count({ where: { role: 'MENTOR', isActive: true } }),
        ...
        database_1.default.learningModule.count({ where: { isActive: true } }),
        database_1.default.exerciseSubmission.count(),
        database_1.default.mentorshipSession.count(),
        database_1.default.mentorshipSession.count({ where: { status: 'COMPLETED' } }),
    ]);
    return {
        users: {
            totalStudents,
...
const getStudentPerformanceAnalytics = async (filters) => {
    ...
    const topStudents = await database_1.default.studentProfile.findMany({
        where: whereClause,
        include: {
    ...
    const studentsWithAvgScore = topStudents
        .map((student) => {
        const scores = student.progress.map((p) => p.score).filter((s) => s !== null);
    ...
    const allProgress = await database_1.default.progress.findMany({
        where: {
            student: whereClause,
            score: { not: null },
        },
    ...
    const allScores = allProgress.map((p) => p.score).filter((s) => s !== null);
...
exports.getStudentPerformanceAnalytics = getStudentPerformanceAnalytics;
// ─────────────────────────────────────────────────────────────
// MODULE ENGAGEMENT ANALYTICS
// ─────────────────────────────────────────────────────────────
const getModuleEngagementAnalytics = async () => {
    ...
    const moduleStats = modules.map((module) => {
        ...
        const scores = module.progress.map((p) => p.score).filter((s) => s !== null);
        const avgScore = scores.length > 0 ? scores.reduce((a, b) => a + b, 0) / scores.length : 0;
        return {
            id: module.id,
            title: module.title,
    ...
    const exerciseTypes = ['LISTENING', 'SPEAKING', 'READING', 'WRITING', 'DIGITAL_LITERACY'];
    const engagementByType = await Promise.all(exerciseTypes.map(async (type) => {
        const count = await database_1.default.exerciseSubmission.count({
...
exports.getModuleEngagementAnalytics = getModuleEngagementAnalytics;
// ─────────────────────────────────────────────────────────────
// MENTOR EFFECTIVENESS ANALYTICS
// ─────────────────────────────────────────────────────────────
const getMentorEffectivenessAnalytics = async () => {
    const mentors = await database_1.default.mentorProfile.findMany({
        include: {
            user: { select: { email: true } },
            sessions: {
                select: {
...
exports.getMentorEffectivenessAnalytics = getMentorEffectivenessAnalytics;
// ─────────────────────────────────────────────────────────────
// PROGRESS OVER TIME (CHART DATA)
// ─────────────────────────────────────────────────────────────
const getProgressOverTime = async (studentId, days = 30) => {
...
exports.getProgressOverTime = getProgressOverTime;
// ─────────────────────────────────────────────────────────────
