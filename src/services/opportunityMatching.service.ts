...
export const matchOpportunitiesForStudent = async (studentId: string) => {
  ...
  const progressScores = student.progress.map((p: any) => p.score).filter((s: any) => s !== null);
  ...
  matchedOpportunities.sort((a: any, b: any) => b.matchScore - a.matchScore);

  // Return top matches (score > 50)
  const topMatches = matchedOpportunities.filter((o: any) => o.matchScore > 50);

  await logAudit(studentId, 'OPPORTUNITY_MATCH_VIEW', {
...
const calculateMatchScore = (
  ...
  if (opportunity.requiredSkills && opportunity.requiredSkills.length > 0) {
    const matchedSkills = opportunity.requiredSkills.filter((reqSkill: string) =>
...
const extractSkillsFromProgress = (progress: any[]): string[] => {
  ...
  const hasDigitalLiteracy = progress.some((p: any) => 
    p.module?.type === 'DIGITAL_LITERACY'
  );
  ...
  const hasCommunicationModules = progress.some((p: any) =>
    ['SPEAKING', 'WRITING', 'LISTENING'].includes(p.module?.type)
  );
...
export const getTopPerformersForOpportunity = async (
  ...
  matchedStudents.sort((a: any, b: any) => b.matchScore - a.matchScore);
  ...
  return {
    ...
    totalMatched: matchedStudents.filter((s: any) => s.matchScore > 50).length
  };
};
...
export const recommendOpportunitiesByType = async (
  studentId: string,
  type: 'JOB' | 'INTERNSHIP' | 'SCHOLARSHIP' | 'UNIVERSITY' | 'TRAINING'
) => {
  const allMatches = await matchOpportunitiesForStudent(studentId);

  const filteredOpportunities = allMatches.opportunities.filter(
    (opp: any) => opp.type === type
  );
